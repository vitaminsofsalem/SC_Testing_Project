 ...
 * @returns {any} response that has been recently cached
 */
...
const getRequestCached = async (url) => {
  ...
  try {
    ...
    if (checkIfStale(cacheObj)) {
      const res = cacheResponse(url, CACHE_LIFETIME_SECONDS * 1000, filePath); //**multiply by 1000 to convert to milliseconds
      return res;
    }
